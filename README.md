...
```bash
# 唯一短前缀，歧义时报候选
 tasktree show d53f4ce2

# 创建持久别名；slug 禁纯 hex，避免和 hash 前缀混淆
 echo "[task] 文档收口" | tasktree add --slug docs-close
 tasktree show docs-close
...
`@last` / `@1` / `@2` 是 `.tasktree/selection-state.json` 下的目录级共享缓存，不是 durable fact，也不是 per-agent 私有 cursor；`--format json` 不读写这个缓存。
