...
| `behavior` | release CLI 黑盒 scope、cursor、三关系正交、并发完成态、manifest smoke 与 reviewed text snapshots | `cargo test --release --test behavior_harness` | Fast, Full | 每场景独立 temp project；固定 `NO_COLOR`/`TZ`；timeout 420 秒含冷编译 | 8 tests，测试约 90 秒 | test report；`tests/behavior_harness.rs`, `tests/behavior/*` |
...
AsyncExec gate 每成功启动一个 suite，立即把 canonical Handle 追加到
`runs.jsonl`；每行就是 `async-exec/handle@1`，整个文件可直接作为
`inspect-many --inputs` / `cancel-many --inputs` 的输入。不能等最终 report 才
公开句柄。manifest 只记录本次调用实际提交的 runs，不推导父子所有权，也不引入
递归取消语义。
