...
mnema 是多 agent 协作的语义拓扑基座，也是给人类和 LLM 共同消费的 append-only journal。它保存可验证的事实、关系和状态变化，并提供确定性的递归投影与查询。Agent、模型和 harness 可以更换或消失；journal 中的语义拓扑仍可被后继者重放、发现、审阅和接续。

核心分工只有三层：

| 层 | 负责 | 不负责 |
|---|---|---|
| mnema | 记录、校验、索引、投影、增量读取 | 判断任务含义、总结历史、替用户决策 |
...
5. **默认视野向下闭包。** 当前根之外的 parent、refs 与 `depends-on` 只是未展开出口；跨树发现必须显式发生。
...
Journal 是一组 strand 的持久化容器，也是投影中的虚拟根。所有当前没有有效 `belongs-to` parent 的 strand，都是这个虚拟根的直接投影孩子；`JournalScope` 是虚拟根的完整向下闭包。

虚拟根只存在于 Scope/Projection 语义中：它不是 strand，不产生 entry，不拥有 hash、生命周期或持久化 `belongs-to` edge，也不进入任何 identity 输入。因此“顶层 strand 属于 Journal”是投影事实，不是 journal 中需要追加的关系事实。
...
- journal root strand 或 root entry；
- `effective_parent`；
- “顶层 strand 属于 journal”这一持久化关系；
...
两者是同一个递归操作：`JournalScope = DownwardClosure(VirtualJournalRoot)`，`SubtreeScope(X) = DownwardClosure(X)`。任意 strand 都可成为局部根；不存在顶层、worker、二阶 worker 三套 scope 语义。
