...
主会话（任务不带 strand ID）开始先跑 `tasktree orient`；
能力沿 `tasktree --help` → 子命令 `--help` →
...
- 入口是工单本身，不跑 orient（那是主会话的入口）：
  `tasktree show --id <ID> --digest` 起步，要全文去掉 --digest；
  上游背景 `tasktree depends --id <ID>`；主动检索 `search`。
...
- 每路一条 strand（`tasktree add`）。prompt 只需三样：strand ID、
